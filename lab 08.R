...
nfl %>% 
  ...
  select(win_score, los_score, home_score, away_score) %>% 
  group_by(win_score, los_score) %>% 
  summarise(n = n(), .groups = "drop") %>% 
  ggplot(mapping = aes(x = win_score, y = los_score)) +
  geom_tile(fill = "chartreuse4", color = "darkgreen") +
  geom_text(mapping = aes(label = n), color = "white", size = 2) +
  coord_fixed(xlim = c(0, 30), ylim = c(30, 0)) + 
  ...
  labs(title = "Scorigami",x = "Wining (or tying) score",y = "Losing score") +
  theme(text = element_text(size = 7)) + geom_tile(data = impossible)